...
describe('Transcript component', () => {
  ...
  describe('with valid transcript data', () => {
    ...
    describe('with speaker', () => {
      const newTData = [...tData,
      { begin: 33.3, end: 40.32, speaker: 'Interviewer', text: '<strong>transcript text 3</strong>', tag: 'TIMED_CUE' }];
      beforeEach(() => {
        // Mock custom hook output
        jest.spyOn(hooks, 'useTranscripts').mockImplementation(() => ({
          ...useTranscripts,
      ...
      test('renders speaker', () => {
        const transcriptText = screen.queryAllByTestId('transcript_timed_text')[3];
        expect(transcriptText.innerHTML).toEqual(
          '<u>Interviewer:</u> <strong>transcript text 3</strong>'
        );
        expect(transcriptText).toHaveTextContent('Interviewer: transcript text 3');
      });
    });
  });

  describe('with search turned on (default)', () => {
    ...
    describe('renders', () => {
      const tData = [
        { begin: 1.2, end: 21, text: '[music]', tag: 'TIMED_CUE' },
        { begin: 22.2, end: 26.6, text: 'transcripts text 1', tag: 'TIMED_CUE' },
        { begin: 27.3, end: 31, text: '<strong>transcripts text 2</strong>', tag: 'TIMED_CUE' },
        {
          begin: 33.3, end: 40.32, match: '<strong><span class="ramp--transcript_highlight">transcript</span> text 3</strong>',
          text: '<strong>transcript text 3</strong>', tag: 'TIMED_CUE'
        }];
      beforeEach(() => {
        // Mock custom hook output
        jest.spyOn(hooks, 'useTranscripts').mockImplementation(() => ({
      ...
      test('a highlight for a match', () => {
        const transcriptText = screen.queryAllByTestId('transcript_timed_text')[3];
        expect(transcriptText.innerHTML).toEqual(
          '<strong><span class="ramp--transcript_highlight">transcript</span> text 3</strong>'
        );
        expect(transcriptText).toHaveTextContent('transcript text 3');
      });

      test('plain styled text for not a match', () => {
        const transcriptText = screen.queryAllByTestId('transcript_timed_text')[2];
        expect(transcriptText.innerHTML).toEqual(
          '<strong>transcripts text 2</strong>'
        );
        expect(transcriptText).toHaveTextContent('transcripts text 2');
      });
    });
  });
  ...
  describe('with props', () => {
    ...
    describe('manifestUrl', () => {
      test('with supplementing annotations', () => {
        const props = {
          playerID: 'player-id',
          manifestUrl: 'http://example.com/manifest.json'
        };
      ...
      test('without supplementing annotations', () => {
        const props = {
          playerID: 'player-id',
          manifestUrl: 'http://example.com/manifest.json'
        };
      ...
      test('and transcripts, and transcripts prop takes precedence', () => {
        const props = {
          playerID: 'player-id',
          manifestUrl: 'http://example.com/manifest.json',
          transcripts: [{
    ...
    test('no manifestUrl and empty transcripts', () => {
      const props = {
        playerID: 'player-id',
        transcripts: [],
      };
