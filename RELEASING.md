...
- GitHub CLI installed and setup (https://cli.github.com/) - required by [scripts/update-changelog.sh](scripts/update-changelog.sh) to look up merged PRs and release timestamps
...
3. **Approve the staged publish** using either of the following approaches;
   - Using NPM website with 2FA from [npm Docs](https://docs.npmjs.com/staged-publishing#using-npmjscom-1) OR
   - Using the CLI according to [npm Docs](https://docs.npmjs.com/staged-publishing#using-the-cli-1) with `npm stage approve <stage-id>` locally, using the stage ID from the workflow log or `npm stage list` 
